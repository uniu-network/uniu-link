    ...
    <UiDrawer v-model:open="dialogVisible" :title="isEditing ? '编辑渠道' : '新建渠道'" width="560px">
      <div class="space-y-4">
        ...
        <div class="border-t border-border pt-4">
          ...
          <p class="mt-1 text-xs text-muted-foreground">Prompt 探测会向指定模型发送测试消息，成功响应即代表渠道可用。</p>
        </div>
        <Field label="检测模式"><select v-model="form.health_check_mode" class="form-input w-full" @change="applyHealthCheckMode"><option value="model_list">拉取模型列表</option><option value="prompt" :disabled="!parsedUpstreamModels.length">Prompt 探测</option></select><Help v-if="!parsedUpstreamModels.length">未配置上游模型时只能使用“拉取模型列表”。</Help></Field>
        <div v-if="form.health_check_mode === 'prompt'" class="space-y-4">
          <Field label="探测模型"><select v-model="form.health_check_model" class="form-input w-full"><option v-for="model in parsedUpstreamModels" :key="model" :value="model">{{ model }}</option></select><Help>只能从上游模型列表中选择。</Help></Field>
          <Field label="探测 Prompt"><textarea v-model="form.health_check_prompt" rows="3" class="form-input min-h-24 w-full py-2" /></Field>
          <Field label="Max Tokens"><input v-model.number="form.health_check_max_tokens" type="number" min="1" class="form-input w-full" /><Help>不配置默认 32。</Help></Field>
        </div>
      </div>
...
const defaultHealthCheckPrompt = 'Hi, please respond with a short greeting to confirm you are working.'
const form = ref<any>({ name: '', provider: 'openai', api_type: 'openai', base_url: '', api_key: '', timeout: 30, max_retries: 2, default_weight: 1.0, custom_headers: [], health_check_mode: 'model_list', health_check_model: '', health_check_prompt: defaultHealthCheckPrompt, health_check_max_tokens: 32 })
...
const parsedUpstreamModels = computed(() => parseUpstreamModels())
...
function applyHealthCheckMode() { if (form.value.health_check_mode === 'prompt' && !form.value.health_check_model) form.value.health_check_model = parsedUpstreamModels.value[0] || '' }
...
function openCreate() { isEditing.value = false; form.value = { name: '', provider: 'openai', api_type: 'openai', base_url: '', api_key: '', timeout: 30, max_retries: 2, default_weight: 1.0, custom_headers: [], health_check_mode: 'model_list', health_check_model: '', health_check_prompt: defaultHealthCheckPrompt, health_check_max_tokens: 32 }; upstreamModelsText.value = ''; dialogVisible.value = true }
function openEdit(ch: any) { isEditing.value = true; form.value = { ...ch, api_type: ch.api_type || defaultApiType(ch.provider), api_key: '', custom_headers: objectToHeaderItems(ch.custom_headers), health_check_mode: ch.health_check_mode || 'model_list', health_check_model: ch.health_check_model || '', health_check_prompt: ch.health_check_prompt || defaultHealthCheckPrompt, health_check_max_tokens: ch.health_check_max_tokens || 32 }; upstreamModelsText.value = (ch.upstream_models || []).join('\n'); dialogVisible.value = true }
async function submitForm() { const upstreamModels = parseUpstreamModels(); if (form.value.health_check_mode === 'prompt') { if (!upstreamModels.length) { toast.warning('Prompt 探测需要先配置上游模型'); return } if (!upstreamModels.includes(form.value.health_check_model)) { toast.warning('探测模型必须从上游模型列表中选择'); return } } const data = { ...form.value, upstream_models: upstreamModels, custom_headers: headerItemsToObject(form.value.custom_headers || []), health_check_max_tokens: form.value.health_check_max_tokens || 32 }; if (isEditing.value && !data.api_key) delete data.api_key; try { submitting.value = true; if (isEditing.value) await updateChannel(data.id, data); else await createChannel(data); dialogVisible.value = false; await load() } catch (e: any) { console.error(e); toast.error(e?.response?.data?.detail || '保存失败') } finally { submitting.value = false } }
